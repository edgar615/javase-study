 ...
 */
public class AlarmByteConsumerExample {
...
//        KafkaStream<byte[], byte[]> kafkaStream =
        ...
        Map<String, List<KafkaStream<byte[], byte[]>>> map = consumerConnector.createMessageStreams(topicMap);
        List<KafkaStream<byte[], byte[]>> streams = map.get("my_test3");
        int threadNumber = 0;
        for (KafkaStream<byte[], byte[]> stream : streams) {
            exec.execute(new AlarmByteConsumerRunnable(stream, threadNumber));
            threadNumber ++;
        }
